...
export default function taskRoutes(pool) {
    ...
    try {
      ...
      if (assignee && assignee !== 'all') {
        filteredTasks = tasks.filter(t =>
          t.assignees.some(a => a.id === parseInt(assignee))
        );
      }
  ...
  router.put('/:id/progress', async (req, res) => {
    try {
      ...
      const [current] = await pool.execute('SELECT progress, project_id FROM task WHERE id = ?', [taskId]);
      if (current.length === 0) return res.status(404).json({ success: false, error: 'Task not found' });

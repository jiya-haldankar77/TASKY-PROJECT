...
import { recommendResources, analyzeImpact, getOrgResourceWorkloads } from '../services/schedulingEngine.js';
...
export default function schedulingRoutes(pool) {
  ...
  router.post('/auto-assign', async (req, res) => {
    try {
      ...
      if (tasks[0].status === 'completed') return res.status(400).json({ success: false, error: 'Cannot assign completed task' });
        ...
        if (tasks.length > 0) {
          ...
          if (viableAlternates.length > 0) {
            suggestions.push({
              ...
              from: { id: r.user_id, name: `${r.first_name} ${r.last_name}`, utilization: r.utilization },
              to: { id: viableAlternates[0].user_id, name: viableAlternates[0].name, new_utilization: viableAlternates[0].utilization + (largestTask.expected_effort/viableAlternates[0].max_hours_per_week*100) },

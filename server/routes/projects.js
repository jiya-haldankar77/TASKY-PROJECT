  ...
  router.get('/:id', async (req, res) => {
    try {
      ...
      const [phases] = await pool.execute(
        'SELECT * FROM project_phase WHERE project_id = ? ORDER BY sort_order', [projectId]
      );
    ...
    try {
      ...
      const [result] = await pool.execute(
        `INSERT INTO project (org_id, created_by, name, description, status, priority, color, start_date, end_date)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [orgId, pmId, name, description || null, status || 'planning', priority || 'medium', color || '#1976D2', start_date, end_date]
      );

      const [newProject] = await pool.execute('SELECT * FROM project WHERE id = ?', [result.insertId]);

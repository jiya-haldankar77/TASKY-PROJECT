    ...
    <q-card class="bg-grey-1 column" v-if="projectStore.currentProject">
      ...
      <q-card-section class="bg-white row items-center justify-between q-pa-md shadow-2 z-top" style="flex: 0 0 auto;">
        <div class="row items-center">
          ...
          <q-avatar :color="avatarColor" text-color="white" size="40px" class="q-mr-md text-weight-bold">
            {{ avatarLetter }}
          </q-avatar>
          <div class="column">
            <div class="text-h6 text-weight-bold" style="line-height: 1.2">{{ projectStore.currentProject.name }}</div>
            <div class="text-caption text-grey-7">Project Details</div>
          </div>
        </div>
        
        <div class="row items-center q-gutter-x-sm">
          <q-badge :color="statusColor" :text-color="statusTextColor" :label="formattedStatus" class="q-px-sm q-py-xs text-weight-bold" style="font-size: 11px;" />
          <q-btn unelevated color="primary" label="Add Task" icon="add" @click="showCreateTaskDialog = true" />
        </div>
      </q-card-section>
        ...
        <div class="col-4 column q-gutter-y-md">
          ...
          <q-card flat bordered class="bg-white">
            <q-card-section>
              ...
              <div class="text-body2 text-grey-8 q-mb-md">{{ projectStore.currentProject.description || 'No description provided.' }}</div>
              
              <q-list dense>
                <q-item class="q-px-none">
                  <q-item-section avatar style="min-width: 32px"><q-icon name="priority_high" color="grey-6" size="xs"/></q-item-section>
                  <q-item-section class="text-grey-7">Priority</q-item-section>
                  <q-item-section side>
                    <q-badge :color="`${priorityColor}-1`" :text-color="priorityColor" :label="formattedPriority" class="text-weight-bold" />
                  </q-item-section>
                </q-item>
                <q-item class="q-px-none">
                  <q-item-section avatar style="min-width: 32px"><q-icon name="calendar_today" color="grey-6" size="xs"/></q-item-section>
                  <q-item-section class="text-grey-7">Timeline</q-item-section>
                  <q-item-section side class="text-body2 text-weight-medium">
                    {{ formatDate(projectStore.currentProject.start_date) }} - {{ formatDate(projectStore.currentProject.end_date) }}
                  </q-item-section>
                </q-item>
                <q-item class="q-px-none">
                  <q-item-section avatar style="min-width: 32px"><q-icon name="hourglass_empty" color="grey-6" size="xs"/></q-item-section>
                  <q-item-section class="text-grey-7">Time Left</q-item-section>
                  <q-item-section side class="text-body2 text-weight-medium" :class="`text-${daysLeftColor}`">
              ...
              <div class="q-mt-md">
                <div class="row justify-between text-caption q-mb-xs">
                  ...
                  <span class="text-weight-bold">{{ projectStore.currentProject.progress || 0 }}%</span>
                </div>
                <q-linear-progress :value="(projectStore.currentProject.progress || 0) / 100" :color="progressColor" size="8px" class="rounded-borders" />
              </div>
            </q-card-section>
          </q-card>
          ...
          <q-card flat bordered class="bg-white">
            <q-card-section>
              ...
              <q-list v-if="projectStore.currentProject.team && projectStore.currentProject.team.length > 0">
                <q-item v-for="member in projectStore.currentProject.team" :key="member.id" class="q-px-none">
                  ...
                  <q-item-section>
                    <q-item-label class="text-weight-medium text-body2">{{ member.first_name }} {{ member.last_name }}</q-item-label>
                    <q-item-label caption>{{ member.role_name }}</q-item-label>
                  </q-item-section>
                  <q-item-section side>
                    <q-btn flat round dense icon="mail" color="grey-5" size="sm" :href="`mailto:${member.email}`" v-if="member.email" />
                  </q-item-section>
                </q-item>
              </q-list>
              <div v-else class="text-caption text-grey text-center q-pa-md">No team members assigned yet.</div>
            </q-card-section>
          </q-card>
        </div>
        ...
        <div class="col-8">
          <q-card flat bordered class="bg-white full-height flex column">
            <q-card-section class="row items-center justify-between q-pb-none">
              <div class="text-subtitle1 text-weight-bold">Tasks ({{ projectStore.currentProject.tasks?.length || 0 }})</div>
            ...
            <q-card-section class="q-pt-md" style="flex: 1 1 0; overflow-y: auto;">
              <div v-if="filteredTasks.length > 0" class="q-gutter-y-sm">
                <!-- Task Items -->
                <q-card v-for="task in filteredTasks" :key="task.id" flat bordered class="cursor-pointer task-card" @click="openTask(task)">
                  <q-card-section class="q-pa-sm">
                    <div class="row items-center">
                      <div class="col-1 text-center">
                        <q-icon v-if="task.status === 'completed'" name="check_circle" color="green" size="sm" />
                        <q-circular-progress v-else-if="task.status === 'in-progress'" show-value class="text-blue text-weight-bold" :value="task.progress" size="28px" color="blue" track-color="grey-3" style="font-size: 10px;">
                          {{ task.progress }}
                        </q-circular-progress>
                        <q-icon v-else-if="task.status === 'blocked'" name="cancel" color="red" size="sm" />
                        <q-icon v-else name="radio_button_unchecked" color="grey-5" size="sm" />
                      </div>
                      
                      <div class="col-7 q-pl-sm">
                        <div class="text-weight-medium" :class="{'text-strike text-grey-6': task.status === 'completed'}">{{ task.title }}</div>
                        <div class="text-caption text-grey-7 text-ellipsis" style="max-width: 100%; overflow: hidden; white-space: nowrap;">{{ task.description || 'No description' }}</div>
                      </div>
                      
                      <div class="col-2 row items-center justify-end">
                        <template v-if="task.assignees && task.assignees.length > 0">
                           <q-avatar v-for="(a, i) in task.assignees.slice(0, 2)" :key="a.id" size="24px" class="overlapping-avatar" :style="`margin-left: ${i === 0 ? 0 : '-8px'}; border: 2px solid white;`">
                      ...
                      <div class="col-2 column items-end justify-center">
                        <q-badge :color="`${getTaskPriorityColor(task.priority)}-1`" :text-color="getTaskPriorityColor(task.priority)" :label="task.priority" class="q-mb-xs" style="font-size: 9px;" />
                        <div class="text-caption text-weight-medium" :class="{'text-red': task.status !== 'completed' && new Date(task.deadline) < new Date()}">
...
const avatarLetter = computed(() => {
  return projectStore.currentProject?.name ? projectStore.currentProject.name.charAt(0).toUpperCase() : 'P';
});
...
const statusColor = computed(() => {
  ...
  if (s === 'all-tasks-complete') return 'green-2';
...
const statusTextColor = computed(() => {
  ...
  if (s === 'all-tasks-complete') return 'green-8';

<template>
  <div class="table-container position-relative" style="height: 100%; min-height: 0; overflow: hidden; display: flex; flex-direction: column;">
    <q-table
      :rows="filteredTasks"
      :columns="columns"
      row-key="id"
    ...
    >
      ...
      <template v-slot:body-cell-task="props">
        <q-td :props="props" class="cursor-pointer" @click="$emit('view', props.row)">
          <div class="column">
            ...
            <div class="text-caption text-grey-6" style="font-size: 11px; max-width: 250px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-top: 2px;">{{ props.row.description || 'No description' }}</div>
          </div>
        </q-td>
      </template>
      ...
      <template v-slot:body-cell-project="props">
        <q-td :props="props" style="width: 180px;">
          <div class="row items-center no-wrap cursor-pointer" @click="$emit('view', props.row)">
            <q-avatar :color="props.row.project_color || 'primary'" text-color="white" size="24px" class="q-mr-sm" style="font-weight: bold; font-size: 11px;">
      ...
      <template v-slot:body-cell-priority="props">
        <q-td :props="props">
          <q-badge :color="`${getPriorityColor(props.row.priority)}-1`" :text-color="getPriorityColor(props.row.priority)" :label="formatName(props.row.priority)" class="q-px-sm q-py-xs text-weight-bold rounded-borders" style="font-size: 10px;" />
        </q-td>
      </template>
      ...
      <template v-slot:body-cell-status="props">
        <q-td :props="props">
          <q-badge :color="`${getStatusColor(props.row.status)}-1`" :text-color="getStatusColor(props.row.status)" :label="formatName(props.row.status)" class="q-px-sm q-py-xs text-weight-bold rounded-borders" style="font-size: 10px;" />
        </q-td>
      </template>
      ...
      <template v-slot:body-cell-progress="props">
        <q-td :props="props">
          <div class="column" style="width: 80px;">
            <div class="text-weight-bold text-grey-8 q-mb-xs" style="font-size: 11px;">{{ props.row.progress || 0 }}%</div>
            <q-linear-progress :value="(props.row.progress || 0) / 100" :color="getProgressColor(props.row.progress)" size="3px" class="rounded-borders" />
          </div>
        </q-td>
      </template>
        ...
        <q-td :props="props">
          <div class="row items-center justify-center">
             <template v-if="props.row.assignees && props.row.assignees.length > 0">
               <q-avatar v-for="(member, i) in props.row.assignees.slice(0, 2)" :key="member.id" size="24px" class="overlapping-avatar" :style="`margin-left: ${i === 0 ? 0 : '-8px'}; border: 2px solid white;`">
      ...
      <template v-slot:body-cell-deadline="props">
        <q-td :props="props">
          <div class="column">
            <div :class="['text-weight-bold', isOverdue(props.row) ? 'text-red' : 'text-grey-8']" style="font-size: 12px;">{{ formatDate(props.row.deadline) }}</div>
            <div v-if="props.row.status !== 'completed'" :class="['text-caption', isOverdue(props.row) ? 'text-red' : 'text-grey-6']" style="font-size: 10px;">
      ...
      <template v-slot:body-cell-actions="props">
        <q-td :props="props">
          <div class="row items-center justify-center q-gutter-x-xs no-wrap">
            <q-btn flat round dense icon="visibility" color="indigo" size="10px" class="bg-indigo-1" @click.stop="$emit('view', props.row)">
              <q-tooltip>View Details</q-tooltip>
            </q-btn>
            <q-btn flat round dense icon="more_vert" color="grey-7" size="10px" class="bg-grey-2" @click.stop>
              <q-menu>
      ...
      <template v-slot:bottom>
        <div class="row items-center justify-between text-grey-7 full-width q-py-xs" style="font-size: 13px;">
          <div>Showing {{ showingStart }} to {{ showingEnd }} of {{ filteredTasks.length }} tasks</div>
          <div class="row items-center q-gutter-x-sm">
            <span>Rows per page:</span>
            <q-select v-model="pagination.rowsPerPage" outlined dense :options="[5, 10, 20]" class="q-mr-md bg-white" style="width: 70px;" rounded @update:model-value="pagination.page = 1" />
...
const totalPages = computed(() => Math.max(1, Math.ceil(filteredTasks.value.length / pagination.value.rowsPerPage)));
const showingStart = computed(() => filteredTasks.value.length === 0 ? 0 : (pagination.value.page - 1) * pagination.value.rowsPerPage + 1);
const showingEnd = computed(() => Math.min(pagination.value.page * pagination.value.rowsPerPage, filteredTasks.value.length));
</script>

    ...
    <div v-else-if="legendItems.length > 0" class="row items-center no-wrap">
      ...
      <div class="donut-chart q-mr-md" :style="donutGradient">
        <div class="donut-hole">
          <div class="text-caption text-grey-6" style="font-size: 10px; line-height: 1.2;">Total</div>
          <div class="text-weight-bold" style="font-size: 15px; color: #333; line-height: 1.2;">{{ totalHours }}h</div>
        </div>
      </div>
      ...
      <div class="column flex-1" style="gap: 8px;">
        <div v-for="(item, index) in legendItems" :key="index" class="row items-center justify-between no-wrap">
          ...
          <div class="text-weight-bold text-grey-8" style="font-size: 11px;">{{ item.value }}h ({{ item.percent }}%)</div>
        </div>
      </div>
    </div>
...
const legendItems = computed(() => {
  ...
  const items = Object.entries(roleMap).map(([role, hours]) => {
    ...
    const color = Object.keys(roleColors).find(k => rKey.includes(k)) ? roleColors[Object.keys(roleColors).find(k => rKey.includes(k))!] : roleColors.default;

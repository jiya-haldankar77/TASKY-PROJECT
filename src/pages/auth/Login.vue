  ...
  try {
    ...
    if (result.success && result.user) {
      ...
      if (result.user.role === 'pm') {
        ...
        // Redirect employees to separate employee 2 project with auth token
        const authToken = authStore.token || 'token-' + Date.now()
        const userData = encodeURIComponent(JSON.stringify(result.user))
        window.location.href = `http://localhost:9004/?token=${authToken}&user=${userData}`
      }
    } else {

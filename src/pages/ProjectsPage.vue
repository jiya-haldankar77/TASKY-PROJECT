  ...
  <q-page class="q-pa-md text-black" style="height: 100vh; max-height: 100vh; min-height: 0 !important; display: flex; flex-direction: column; background-color: #f8f9fa;">
    ...
    <div class="row items-start justify-between q-mb-md" style="flex: 0 0 auto;">
      ...
      <div class="column items-end">
        ...
        <q-btn unelevated color="indigo" icon="add" label="New Project" no-caps class="rounded-borders" @click="openCreateDialog" />
      </div>
    </div>
    ...
    <div class="row items-center justify-between q-mb-md" style="flex: 0 0 auto;">
      <q-input v-model="filters.search" outlined dense rounded bg-color="white" placeholder="Search projects..." style="width: 250px;" @update:model-value="applyFilters">
      ...
      <div class="row items-center q-gutter-x-sm">
        <q-select v-model="filters.status" outlined dense :options="statusOptions" style="width: 160px;" bg-color="white" rounded emit-value map-options @update:model-value="applyFilters" />
        <q-select v-model="filters.priority" outlined dense :options="priorityOptions" style="width: 160px;" bg-color="white" rounded emit-value map-options @update:model-value="applyFilters" />
        <q-select v-model="filters.sort" outlined dense :options="sortOptions" style="width: 170px;" bg-color="white" rounded emit-value map-options @update:model-value="applyFilters" />
        <q-btn v-if="hasActiveFilters" flat round dense icon="clear" color="grey" @click="clearFilters">
    ...
    <!-- Project List -->
    <div class="q-mb-md position-relative" style="flex: 1 1 0; overflow-y: auto; min-height: 0; padding-right: 4px;">
    ...
    <!-- Pagination Footer -->
    <div class="row items-center justify-between text-grey-7" style="flex: 0 0 auto; font-size: 13px;" v-if="projectStore.projects.length > 0">
      <div>Showing {{ showingStart }} to {{ showingEnd }} of {{ projectStore.projects.length }} projects</div>
      <div class="row items-center q-gutter-x-sm">
        <span>Rows per page:</span>
        <q-select v-model="rowsPerPage" outlined dense :options="[5, 10, 20]" class="q-mr-md bg-white" style="width: 70px;" rounded @update:model-value="currentPage = 1" />
    ...
    <!-- Dialogs -->
    <CreateProjectDialog v-model="showCreateDialog" :project-to-edit="projectToEdit" @saved="projectStore.fetchProjects(filters)" />
...
const hasActiveFilters = computed(() => {
  return filters.value.search !== '' || filters.value.status !== 'all' || filters.value.priority !== 'all';
});
...
const totalPages = computed(() => Math.max(1, Math.ceil(projectStore.projects.length / rowsPerPage.value)));
const showingStart = computed(() => projectStore.projects.length === 0 ? 0 : (currentPage.value - 1) * rowsPerPage.value + 1);
const showingEnd = computed(() => Math.min(currentPage.value * rowsPerPage.value, projectStore.projects.length));

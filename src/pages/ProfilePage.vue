<template>
  <q-page class="q-pa-md text-black" style="height: 100vh; max-height: 100vh; min-height: 0 !important; display: flex; flex-direction: column; background-color: #f8f9fa;">
    ...
    <!-- Header / Identity -->
    <div class="row items-center q-mb-xl q-pa-md bg-white rounded-borders shadow-1" style="flex: 0 0 auto;">
      <q-avatar size="100px" class="q-mr-lg shadow-2" color="indigo-1" text-color="indigo">
        ...
        <span class="text-h3" v-else-if="authStore.currentUser">{{ authStore.currentUser.firstName.charAt(0) }}{{ authStore.currentUser.surname.charAt(0) }}</span>
      </q-avatar>
      <div class="column" v-if="authStore.currentUser">
        <div class="text-h4 text-weight-bold">{{ authStore.currentUser.firstName }} {{ authStore.currentUser.surname }}</div>
        <div class="text-h6 text-grey-8 text-capitalize">{{ authStore.currentUser.role === 'pm' ? 'Project Manager' : 'Employee' }}</div>
        <div class="text-caption text-grey-6 row items-center q-mt-xs">
          <q-icon name="email" class="q-mr-xs" /> {{ authStore.currentUser.email }}
          <q-icon name="phone" class="q-ml-md q-mr-xs" v-if="authStore.currentUser.phone" /> {{ authStore.currentUser.phone }}
        </div>
      </div>
      <q-space />
      <q-btn unelevated color="indigo" icon="edit" label="Edit Profile" no-caps @click="openEditDialog" />
    </div>

    <!-- Personal Metrics -->
    <div class="row q-col-gutter-md q-mb-lg" style="flex: 0 0 auto;">
      <div class="col-4">
        <StatCard title="Projects Managed" :value="projectStore.projects.length.toString()" color="indigo" icon="folder_special" caption="Active in your portfolio" />
      </div>
      <div class="col-4">
        <StatCard title="Average Completion Rate" :value="`${avgProgress}%`" color="green" icon="verified" caption="Historical performance">
          <template v-slot:caption>
            <span class="text-green"><q-icon name="arrow_upward" size="10px" /> 2%</span> from last quarter
          </template>
        </StatCard>
      </div>
      <div class="col-4">
        <StatCard title="Total Team Size" :value="totalTeamSize.toString()" color="blue" icon="groups" caption="Resources across projects" />
      </div>
    </div>
    ...
    <div class="column" style="flex: 1 1 0; min-height: 0;">
      ...
      <div v-else style="flex: 1 1 0; overflow-y: auto; padding-right: 8px;">
        <ProjectListCard v-for="project in projectStore.projects" :key="project.id" :project="project" />
      ...
      <q-card style="width: 400px; max-width: 90vw;">
        ...
        <q-card-section class="q-pt-md">
          <q-form @submit="saveProfile" class="q-gutter-md">
            <q-input outlined v-model="editForm.firstName" label="First Name" lazy-rules :rules="[val => !!val || 'First Name is required']" />
            <q-input outlined v-model="editForm.surname" label="Last Name" lazy-rules :rules="[val => !!val || 'Last Name is required']" />
            <q-input outlined v-model="editForm.email" type="email" label="Email" lazy-rules :rules="[val => !!val || 'Email is required']" />

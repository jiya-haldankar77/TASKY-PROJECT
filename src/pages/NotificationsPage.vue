<template>
  <q-page class="q-pa-md text-black" style="height: 100vh; max-height: 100vh; min-height: 0 !important; display: flex; flex-direction: column; background-color: #f8f9fa;">
    ...
    <q-card flat bordered class="bg-white" style="flex: 1 1 0; display: flex; flex-direction: column;">
      ...
      <div v-else-if="notificationStore.notifications.length > 0" style="overflow-y: auto; flex: 1 1 0;">
        <q-list separator>
          <q-item v-for="notif in notificationStore.notifications" :key="notif.id" class="q-py-md" :class="{'bg-blue-1': !notif.is_read}">
            <q-item-section avatar top>
              <q-avatar :color="getIconColor(notif.type)" text-color="white" :icon="getIcon(notif.type)" />
            </q-item-section>

    ...
    async fetchCalendarData(start?: string, end?: string) {
      ...
      try {
        ...
        const [taskRes, availRes, leaveRes] = await Promise.all([
          fetch(`http://localhost:3001/api/pm/calendar/tasks${query}`, { headers: this.getHeaders() }),
          fetch(`http://localhost:3001/api/pm/calendar/availability${query}`, { headers: this.getHeaders() }),
          fetch(`http://localhost:3001/api/pm/calendar/leave${query}`, { headers: this.getHeaders() })
        ]);
